...
public class Scene implements SceneGraphNode {
	...
	public void display(GLAutoDrawable drawable) {
		// this is never called (unless you change the code)
	...
	private void drawAll(GLAutoDrawable drawable, FlatMatrix4d Pinv, FlatMatrix4d Vinv, ViewMode mode ) {
		...
		fa.draw( gl ); // world axis
		if ( mode == ViewMode.NDC ) { gl.glDisable( GL2.GL_LIGHTING ); }
		
		gl.glPushMatrix();  // DRAW THE LOOK AT POINT
		...
		// DRAW THE UP VECTOR
		final FancyArrow arrow = new FancyArrow();
		arrow.setFrom( lookAt.x, lookAt.y, lookAt.z );
		arrow.setTo( lookAt.x + up.x, lookAt.y + up.y, lookAt.z + up.z );
		arrow.draw(gl);
